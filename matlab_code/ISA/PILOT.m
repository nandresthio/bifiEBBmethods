...
errorfcn = @(alpha,Xbar,n,m) nanmean(nanmean((Xbar-(reshape(alpha((2*n)+1:end),m,2)*... % B,C
                                                   ...
                                                   *Xbar(:,1:n)')').^2,1),2);
% Nico: Take the average error between the features and the algorithm
% performance
% errorfcn = @(alpha,Xbar,n,m) (0.5 * nanmean(nanmean((Xbar(:,1:n)-(reshape(alpha((2*n)+1:(2*n+2*n)),n,2)*reshape(alpha(1:2*n),2,n)*Xbar(:,1:n)')').^2,1),2) + ...
%                                 0.5 * nanmean(nanmean((Xbar(:,(n+1):end)-(reshape(alpha((2*n+2*n)+1:(2*n+2*m)),m-n,2)*reshape(alpha(1:2*n),2,n)*Xbar(:,1:n)')').^2,1),2));
