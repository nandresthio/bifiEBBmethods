...
Kriging::Kriging(Function* ebbFunction, AuxSolver* auxSolver, int sampleBudget, int randomSeed, bool printInfo, bool functionScaling):
...
void Kriging::scalePoint(VectorXd &point){
	if(functionScaling_){
		for(int i = 0; i < (int)point.size(); i++){
			point(i) = 1*(point(i) - ebbFunction_->lowerBound_[i]) / (ebbFunction_->upperBound_[i] - ebbFunction_->lowerBound_[i]);
		}
	}
}

void Kriging::scalePoints(vector<VectorXd> &points){
	for(int i = 0; i < (int)points.size(); i++){
		scalePoint(points[i]);
	}
}

void Kriging::unscalePoint(VectorXd &point){
	if(functionScaling_){
		for(int i = 0; i < (int)point.size(); i++){
			point(i) = 1*point(i) * (ebbFunction_->upperBound_[i] - ebbFunction_->lowerBound_[i]) + ebbFunction_->lowerBound_[i];
		}
	}
}

void Kriging::unscalePoints(vector<VectorXd> &points){
	for(int i = 0; i < (int)points.size(); i++){
		unscalePoint(points[i]);
	}
}

double Kriging::scaleObservation(double value){
...
void Kriging::scaleObservations(vector<double> &observations){
...
double Kriging::unscaleObservation(double value){
	if(!functionScaling_){return value;}
	if(maxObservation_ == DBL_MAX || minObservation_ == DBL_MAX){
...
void Kriging::unscaleObservations(vector<double> &observations){
	for(int i = 0; functionScaling_ && i < (int)observations.size(); i++){
...
void Kriging::saveSample(vector<VectorXd> sample, vector<double> sampleVals){
...
void Kriging::generateSample(){
	...
	vector<VectorXd> points = sampleGenerator_->randomLHS(sampleBudget_);
	if(printInfo_){printf("\n");}
	
	// sampleGenerator_->morrisMitchellLocalOptimal(sampledPoints_);
	// if(printInfo_){printf("\n");}
	vector<double> pointsVals = ebbFunction_->evaluateMany(sampledPoints_);
	saveSample(points, pointsVals);
...
double Kriging::surfaceValue(VectorXd &x, bool pointIsScaled, bool scaleOutput){
	VectorXd xCopy = x;
	if(!pointIsScaled){scalePoint(xCopy);}
	auto data = meanVarianceCalculator(xCopy);
	double s_x = get<0>(data);
	if(scaleOutput){s_x = unscaleObservation(s_x);}
	return s_x;
}

double Kriging::uncertainty(VectorXd &x, bool pointIsScaled, bool scaleOutput){
	VectorXd xCopy = x;
	if(!pointIsScaled){scalePoint(xCopy);}
	auto data = meanVarianceCalculator(xCopy);
...
double Kriging::expectedImprovement(VectorXd &x, bool pointIsScaled, bool scaleOutput){
	VectorXd xCopy = x;
	if(!pointIsScaled){scalePoint(xCopy);}
	auto data = meanVarianceCalculator(xCopy);
	double s_x = get<0>(data);
	...
	if(scaleOutput || !functionScaling_){
...
CoKriging::CoKriging(BiFidelityFunction* biFunction, AuxSolver* auxSolver, int highFiSampleBudget, int lowFiSampleBudget, int randomSeed, bool printInfo, bool functionScaling):
...
void CoKriging::scaleTwoObservations(vector<double> &observations, vector<double> &observationsLow){
	scaleObservations(observations);
	for(int i = 0; i < (int)observationsLow.size(); i++){
		observationsLow[i] = scaleObservation(observationsLow[i]);
	}
...
void CoKriging::unscaleTwoObservations(vector<double> &observations, vector<double> &observationsLow){
	unscaleObservations(observations);
	unscaleObservations(observationsLow);
}

void CoKriging::saveSample(vector<VectorXd> sample, vector<double> sampleVals, vector<VectorXd> sampleLow, vector<double> sampleValsLow){
	sampledPoints_ = sample;
	sampledPointsValues_ = sampleVals;
	lowFiKriging_->sampledPoints_ = sampleLow;
	lowFiKriging_->sampledPointsValues_ = sampleValsLow;
	scalePoints(sampledPoints_);
	scalePoints(lowFiKriging_->sampledPoints_);
	scaleTwoObservations(sampledPointsValues_, lowFiKriging_->sampledPointsValues_);
}

void CoKriging::sampleAtLocation(VectorXd point){
	// First of all scale the point
	VectorXd scaledCopy = point;
	scalePoint(scaledCopy);
	// First check that the point is not repeated
	bool isRepeated = checkIfRepeated(sampledPoints_, scaledCopy);
	bool isRepeatedLow = checkIfRepeated(lowFiKriging_->sampledPoints_, scaledCopy);
	if(isRepeated){
	...
	if(isRepeatedLow){
		printf("Not saving low fi value of point as it is already used by the model!\n");
	}else{
		double valLow = biFunction_->evaluateLow(point);
		lowFiKriging_->sampledPoints_.push_back(scaledCopy);
		valLow = scaleObservation(valLow);
		lowFiKriging_->sampledPointsValues_.push_back(valLow);
	}
}
...
void CoKriging::generateSample(){
	...
	vector<VectorXd> pointsLow = sampleGenerator_->randomLHS(lowFiSampleBudget_);
	// Scale to choose a good subset
	...
	vector<VectorXd> points = sampleGenerator_->morrisMitchellSubset(pointsLow, highFiSampleBudget_);
	unscalePoints(pointsLow);

	// Get function objective values and save
	vector<double> pointsLowVals = biFunction_->evaluateManyLow(pointsLow);
	vector<double> pointsVals = biFunction_->evaluateManyLow(points);
	saveSample(points, pointsVals, pointsLow, pointsLowVals);
